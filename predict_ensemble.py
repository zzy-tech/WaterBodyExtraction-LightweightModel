...
def predict_with_ensemble(model, image_path, args, image_index=None):
    ...
    with rasterio.open(image_path) as src:
        ...
        count = src.count
        bands = args.bands if args.bands is not None else list(range(1, count + 1))
        ...
        if height <= args.tile_size and width <= args.tile_size:
            ...
            image = src.read(valid_bands).astype(np.float32)
            normalized_image = normalize_image(image, method='sentinel')
            prediction = predict_on_tile(model, normalized_image, args.device)
        else:
            logger.info(f'图像尺寸较大，按窗口分块处理 ({height}x{width})')
            normalized_image = None
            prediction = process_large_raster(model, src, valid_bands, args)
    
    # 应用CRF后处理
    if args.crf_iterations > 0:
        if normalized_image is None:
            logger.warning('大图窗口模式下跳过 CRF 后处理，避免额外整图内存占用')
        else:
    ...
    if args.save_predictions:
        if normalized_image is None:
            prediction_path = save_prediction_to_geotiff(
                prediction,
                profile,
                args.output_dir,
                filename,
                threshold=args.threshold,
                chunk_rows=args.tile_size * 4,
            )
        else:
    ...
    if args.save_visualization and normalized_image is not None:
        # 检查是否设置了最大可视化数量限制
        ...
        logger.info('大图窗口模式下跳过可视化，避免额外整图内存占用')
...
def process_large_image(model, image, args):
    ...
    return process_large_array(model, image, height, width, args)

def build_tile_positions(height, width, tile_size, overlap):
    """生成滑窗位置列表。"""
    ...
    for i in range(num_tiles_h):
        for j in range(num_tiles_w):
            start_h = min(i * stride, max(height - tile_size, 0))
            end_h = min(start_h + tile_size, height)
            start_w = min(j * stride, max(width - tile_size, 0))
            end_w = min(start_w + tile_size, width)
            positions.append((start_h, end_h, start_w, end_w))
    
    return positions, num_tiles_h, num_tiles_w

def create_weight_map(height, width, overlap, fade_top=True, fade_bottom=True, fade_left=True, fade_right=True):
    """创建用于重叠区域加权平均的权重图。"""
    ...
    if fade_h > 0 and fade_top:
        weight_h[:fade_h] = np.linspace(0, 1, fade_h, dtype=np.float32)[:, np.newaxis]
    if fade_h > 0 and fade_bottom:
        weight_h[-fade_h:] = np.linspace(1, 0, fade_h, dtype=np.float32)[:, np.newaxis]
    
    if fade_w > 0 and fade_left:
        weight_w[:, :fade_w] = np.linspace(0, 1, fade_w, dtype=np.float32)[np.newaxis, :]
    if fade_w > 0 and fade_right:
        weight_w[:, -fade_w:] = np.linspace(1, 0, fade_w, dtype=np.float32)[np.newaxis, :]
    
    return np.maximum(weight_h * weight_w, 1e-6)

def merge_batch_predictions(batch_output, batch_positions, prediction, weight_map, image_height, image_width, overlap):
    """将一批瓦片预测结果回写到整图。"""
    for j, (start_h, end_h, start_w, end_w) in enumerate(batch_positions):
        ...
        weight = create_weight_map(
            ...
            overlap,
            fade_top=start_h > 0,
            fade_bottom=end_h < image_height,
            fade_left=start_w > 0,
            fade_right=end_w < image_width,
        )
        prediction[0, start_h:end_h, start_w:end_w] += output[0] * weight
        weight_map[start_h:end_h, start_w:end_w] += weight

def finalize_prediction(prediction, weight_map):
    """对重叠区域做归一化。"""
    mask = weight_map > 0
    prediction[0, mask] /= weight_map[mask]
    return prediction

def create_disk_backed_array(output_dir, filename, suffix, shape, dtype=np.float32):
    """创建磁盘映射数组，避免大图整图常驻内存。"""
    os.makedirs(output_dir, exist_ok=True)
    base_name = os.path.splitext(os.path.basename(filename))[0]
    mmap_path = os.path.join(output_dir, f'{base_name}_{suffix}.mmap')
    return np.memmap(mmap_path, dtype=dtype, mode='w+', shape=shape)

def process_large_array(model, image, height, width, args):
    """对已加载数组执行流式滑窗推理。"""
    positions, num_tiles_h, num_tiles_w = build_tile_positions(height, width, args.tile_size, args.overlap)
    logger.info(f'分块: {num_tiles_h}x{num_tiles_w} 瓦片')
    
    prediction = np.zeros((1, height, width), dtype=np.float32)
    weight_map = np.zeros((height, width), dtype=np.float32)
    
    for i in tqdm(range(0, len(positions), args.batch_size), desc='处理瓦片'):
        batch_positions = positions[i:i + args.batch_size]
        batch_tiles = [
            image[:, start_h:end_h, start_w:end_w]
            for start_h, end_h, start_w, end_w in batch_positions
        ]
        batch_tensor = torch.from_numpy(np.stack(batch_tiles, axis=0)).float().to(args.device)
        ...
        merge_batch_predictions(batch_output, batch_positions, prediction, weight_map, height, width, args.overlap)
    
    return finalize_prediction(prediction, weight_map)

def process_large_raster(model, src, bands, args):
    """对超大 GeoTIFF 按窗口读取并推理，避免整图加载到内存。"""
    height, width = src.height, src.width
    positions, num_tiles_h, num_tiles_w = build_tile_positions(height, width, args.tile_size, args.overlap)
    logger.info(f'分块: {num_tiles_h}x{num_tiles_w} 瓦片')
    
    filename = os.path.basename(src.name)
    prediction = create_disk_backed_array(args.output_dir, filename, 'prediction_accum', (1, height, width))
    prediction[:] = 0
    weight_map = create_disk_backed_array(args.output_dir, filename, 'weight_accum', (height, width))
    weight_map[:] = 0
    ...
    for i in tqdm(range(0, len(positions), args.batch_size), desc='处理瓦片'):
        ...
        for start_h, end_h, start_w, end_w in batch_positions:
            window = Window(start_w, start_h, end_w - start_w, end_h - start_h)
            tile = src.read(bands, window=window).astype(np.float32)
            batch_tiles.append(normalize_image(tile, method='sentinel'))
...
def save_prediction_to_geotiff(prediction, profile, output_dir, filename, threshold=None, chunk_rows=1024):
    """保存预测结果为GeoTIFF文件"""
    # 创建输出目录
    os.makedirs(output_dir, exist_ok=True)
    ...
    with rasterio.open(output_path, 'w', **profile) as dst:
        if threshold is None:
            ...
            for row_start in range(0, height, chunk_rows):
                ...
                block = (prediction[row_start:row_end] > threshold).astype(np.float32)
                dst.write(block, 1, window=Window(0, row_start, block.shape[1], block.shape[0]))
